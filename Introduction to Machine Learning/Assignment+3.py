...
def answer_five():
    ...
    recall_a = np.interp(0.75,precision,recall)
    fpr,tpr,thresholds = roc_curve(y_test,y_predict)
    tpr_a = np.interp(0.16,fpr,tpr)
    return (recall_a,tpr_a)
...
def answer_six():    
    ...
    clf = LogisticRegression()
    GridSearch = GridSearchCV(clf,param_grid=parameters,scoring='recall',cv=3)
    GridSearch.fit(X_train,y_train)
    mean_test_score = GridSearch.cv_results_['mean_test_score']
    mean_test_score = np.reshape(mean_test_score,(5,2))
